...
#' this function
#' The `visSetup` function takes the output from runGORESP and generates objects for drawing GO enrichment networks
...
#' @param fontsize fontsize for node labels; default = 22
#' @param fontface fontface for node labels; default = "Arial"
#' @return Returns two dataframes in a named list for using with visNetwork; Network visualization using vis.js library.
#' nodes has the following columns: id, filename,term, nGenes, nQuery, nOverlap, querySetFraction, geneSetFraction, foldEnrichment, P, FDR, overlapGenes, maxOverlapGeneScore: see ??runGORESP enrichInfo output for explanation of these columns
# visualization columns for visOptions and visNodes: size, formattedLabel, label, color.background,color.border, borderWidthSelected, color.highlight.border, color.highlight.background, color.hover.background, color.hover.border, font.face, shape, font.size, font.bold, borderWidth, labelHighlightBold and
# edges has the columns from, to, overlapCoeff, width and color
#' \describe{
#'   \item{nodes}{A binary indicator of significance: 1 if the score is significant, 0 otherwise.}
...
#'   maxSetSize = 300)
#' vis <-  visSetup(enrichInfo = hresp$enrichInfo,edgeMat = hresp$edgeMat)
#' @export
